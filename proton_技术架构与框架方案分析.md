...
### 11.2 MemPalaceMemoryProvider：生态化空间长期记忆

通过接入开源项目 [MemPalace](https://github.com/MemPalace/mempalace) 的 MCP 插件，Proton 原生支持了强大的空间记忆路由架构。
原有的 `PortalMemoryManager`（基于 Jaccard 相似度的轻量实现）已被彻底移除。

- **空间路由映射**：
  - **Palace (宫殿)**：对应全局共享记忆域（默认 `proton_global_memory`），支持跨域漫游。
  - **Wing (分区)**：映射为 Proton 的多租户隔离层，支持 `per_user`（按用户隔离）或 `per_portal`（按入口隔离）。
  - **Room (房间)**：作为上下文微区（默认 `general`）。
- **生命周期映射**：
  - 对话读取 (Retrieval) ➔ 映射为 `mempalace_search`。
  - 对话写入 (Extraction) ➔ 映射为 `mempalace_add_drawer`。
  - 基础 CRUD ➔ 映射为删除或清空相应的抽屉。
- **降维打击**：相比原生文本相似度检索，MemPalace 提供原生的向量搜索与空间隔离，极大提升了多轮记忆提取的准确率和隐私隔离性。
...
- **目前状况：** 
  - system tool 标记了 `requires_approval/is_dangerous`，但 BuiltinAgentAdapter 里执行 system tool 时并没有统一的 approval gate。
  - **最新进展：** Builtin tool 的 `code` 执行已经通过引入 `src/execution/backends/docker.py` 和 `local.py` 实现了执行后端分层隔离，废弃了原有危险的 `exec()` 方式。

建议：
1. 进一步在后端引入“Tool Execution Policy Engine”的强制拦截器，将需要审批的调用阻塞并抛出至 UI（ExecutionPanel）。

### 15.2 并行执行与共享 context 的竞态风险

- **目前状况：** 
  - **最新进展：** 已经在 `ExecutionContext` 中实现了 `create_child_context(isolate=True)` 以及 `merge_isolated_context` 机制，在 `TreeExecutor` 中通过深拷贝完美消除了并发状态（`shared_state`, `errors`）的数据竞态。
...
1. **统一 Tool 执行与策略/审批**（形成后端强约束与 UI 联动）  
2. **事件协议类型化与版本化**（后端/前端一致演进）  
3. **技能/插件生态的安全与运行时标准化**（包结构、依赖、审计、签名）  
4. **配置体系收敛**（明确优先级、输出生效快照、减少双轨）  
~~5. **并行执行的上下文隔离与合并机制**（已完成）~~
~~6. **code 工具沙箱化**（已完成）~~
