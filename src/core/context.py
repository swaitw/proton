...
if TYPE_CHECKING:
    from ..execution.backends.base import ExecutionBackend
...
class ExecutionContext:
    ...
    def _prune_tool_results(self, messages: List[ChatMessage]) -> None:
        """
        Prune extremely long tool results to avoid polluting the summarization.
        This modifies the messages in-place.
        """
        for msg in messages:
            # We consider messages generated by a tool to be pruned
            # In Proton, this might be msg.role == MessageRole.TOOL or specific content patterns
            # Also handle cases where large text is returned from tools
            is_tool_result = (
                msg.role.value == "tool" or
                getattr(msg, "tool_call_id", None) is not None or
                (msg.role.value == "assistant" and "Tool execution result:" in msg.content)
            )
            if is_tool_result and len(msg.content) > 500:
                msg.content = f"[Tool result omitted for brevity. Length: {len(msg.content)} chars]"
    ...
    def _compress_context(self) -> None:
        ...
        """
        # First prune extremely long tool results from the whole history (except the very last few)
        # to see if that resolves the token pressure without needing a full summarization
        if len(self.messages) > 4:
            self._prune_tool_results(self.messages[1:-2])
        ...
        # If middle is empty after slicing, we don't need to summarize
        if not middle:
            return
        if middle:
